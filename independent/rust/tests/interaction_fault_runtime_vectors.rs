...
fn vector() -> Value {
    ...
    let source = fs::read(root.join("tests/vectors/interaction-fault-runtime.json"))
        .expect("shared interaction fault runtime vector");
    serde_json::from_slice(&source).expect("vector JSON")
}

#[test]
fn independent_interaction_fault_discards_tick_and_applies_shared_policy() {
    let vector = vector();
