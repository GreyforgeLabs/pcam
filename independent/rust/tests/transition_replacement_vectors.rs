...
fn vector() -> Value {
    ...
    let source = fs::read(root.join("tests/vectors/transition-replacement.json"))
        .expect("shared transition replacement vector");
    serde_json::from_slice(&source).expect("vector JSON")
}

#[test]
fn independent_transition_replacement_matches_shared_atomic_outcomes() {
    let vector = vector();
