...
def _vector():
    return json.loads(
        (ROOT / "tests/vectors/interaction-fault-runtime.json").read_text(encoding="utf-8")
    )


def _fault_host(vector):
    contact = vector["fault_tick"]["contacts"][0]
    return HostSnapshot(
        contacts=(
            Contact(
...
def test_python_interaction_fault_discards_tick_and_applies_shared_policy():
    vector = _vector()
