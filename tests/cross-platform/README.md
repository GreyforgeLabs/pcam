...
To produce the missing evidence, run the following on a host whose operating system reports Linux and whose machine architecture reports `aarch64` or `arm64`:
...
The runner executes both implementations and refuses to emit a manifest when they diverge. The resulting `suite_digest` must equal `795bec98fc4e22d2127d64868af5ba1b806a42be62fd40f3fa8b90846b38046d` before the cross-platform gate can change from OPEN.
