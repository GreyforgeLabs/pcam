...
| 38.9 Slot replacement | accepted and paired rejected cases in `vectors/transition-replacement.json`, executed by both complete-state runtimes | implemented directly: success releases and reacquires the slot atomically, while insufficient target resources preserve the source lifecycle, slot ownership, bank, transition serial, and identifier counter exactly; the same vector pins first-wins duplicate-input handling and distinguishes rejected `ON_ATTEMPT` consumption from `ON_ACCEPT` retention |
